...
function parseImageInfo(
  ...
  const imageTypes = metadata.imageTypes || metadata.types;
  const cropSizes = metadata.cropSizes;

  if (typeof imageCount === "number" && imageCount > 0) {
    return {
      count: imageCount,
      types: Array.isArray(imageTypes) ? imageTypes.map(String) : [],
      cropSizes: Array.isArray(cropSizes) ? cropSizes.map(String) : undefined,
    };
  }
  return undefined;
}
...
/**
 * Parse categorization information
 */
function parseCategorizationInfo(metadata: Record<string, unknown>): {
  ...
  const categoryNames = (metadata.categoryNames ||
    metadata.categories ||
    []) as string[];
  const tagNames = (metadata.tagNames || metadata.tags || []) as string[];
