...
import { ActivityLogProps, ActivityItem, UploadProgress } from "./types";
...
export function ActivityLog({
    ...
    importItems.forEach((item) => {
      ...
      if (existingItem && existingItem.type === "upload") {
        ...
        const uploadProgress: UploadProgress = {
          ...
          uploadStatus: existingItem.status as "uploading" | "uploaded" | "failed",
        };
        ...
        // No existing upload item, just add the import item
        allItems.set(item.importId, {
          ...item,
          type: "import",
        });
  ...
  // Debug logging
  console.log("📊 [ACTIVITY_LOG] Current state:", {
    importItemsCount: importItems.length,
    uploadItemsCount: uploadItems.size,
    fileTitlesCount: fileTitles.size,
    ...
    uploadItems: Array.from(uploadItems.values()).map((item) => ({
      importId: item.importId,
      htmlFileName: item.htmlFileName,
      status: item.status,
    })),
    fileTitles: Object.fromEntries(fileTitles),
  });
  ...
  return (
    <div className={`space-y-3 ${className || ""}`}>
      ...
      {htmlFiles.map((htmlFile, index) => {
        const importItem = fileMatchingMap.get(htmlFile) || 
