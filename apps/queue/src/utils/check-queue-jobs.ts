...
async function checkQueueJobs() {
  ...
  try {
    ...
    failedJobs.slice(-5).forEach(job => {
      console.log(`  - ${job.type} (${job.jobId}) for note ${job.noteId} - ${job.errorMessage || 'No error message'}`);
    });
    ...
    const categorizationJobs = allJobs.filter(job => job.type === "PROCESS_CATEGORIZATION");
