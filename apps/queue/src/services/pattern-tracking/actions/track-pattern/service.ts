...
export async function trackPattern(
  ...
  try {
    if (!data.patternRules || data.patternRules.length === 0) {
      logger.log(`[TRACK_PATTERN] No pattern rules provided for job ${data.jobId}`);
      return data;
    }
    
    logger.log(`[TRACK_PATTERN] Starting pattern tracking for job ${data.jobId} with ${data.patternRules.length} rules`);
    ...
    logger.log(`[TRACK_PATTERN] Successfully tracked pattern ${patternId} for job ${data.jobId}`);
    ...
    logger.log(`[TRACK_PATTERN] Failed to track pattern for job ${data.jobId}: ${error}`);
    return {
      ...data,
      metadata: {
        ...data.metadata,
