...
import { ScheduleImagesAction, scheduleImages } from "../../note/schedule-images";
...
describe("ScheduleImagesAction", () => {
  ...
  describe("validateInput", () => {
    ...
    it("should return error for data without noteId", () => {
      ...
      expect(result?.message).toBe(
        "Note ID is required for image scheduling"
      );
    });
    ...
    it("should execute successfully and return the data", async () => {
      ...
      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Starting image scheduling for note: test-note-id"
        )
      );
      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Successfully scheduled image processing for note: test-note-id"
        )
      );
    });
  ...
  describe("scheduleImages function", () => {
    it("should schedule image processing successfully", async () => {
      const result = await scheduleImages(
        mockData,
    ...
    it("should throw error when noteId is missing", async () => {
      ...
      await expect(
        scheduleImages(dataWithoutNoteId, mockDependencies.logger)
      ).rejects.toThrow("No note ID available for image scheduling");
    });
    ...
    it("should handle errors and log them", async () => {
      ...
      await expect(
        scheduleImages(mockData, mockDependencies.logger)
      ).rejects.toThrow("Test error");

      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Failed to schedule image processing: Error: Test error"
        )
      );
    });
  });
