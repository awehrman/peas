...
import {
  ...
  scheduleIngredients,
} from "../../note/schedule-ingredients";
...
describe("ScheduleIngredientsAction", () => {
  ...
  describe("name", () => {
    it("should have the correct action name", () => {
      expect(action.name).toBe(ActionName.SCHEDULE_INGREDIENTS);
    });
  });
  ...
  describe("validateInput", () => {
    ...
    it("should return error for data without noteId", () => {
      ...
      expect(result?.message).toBe(
        "Note ID is required for ingredient scheduling"
      );
    });
    ...
    it("should execute successfully and return the data", async () => {
      ...
      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Starting ingredient scheduling for note: test-note-id"
        )
      );
      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Successfully scheduled ingredient processing for note: test-note-id"
        )
      );
    });
  ...
  describe("scheduleIngredients function", () => {
    it("should schedule ingredient processing successfully", async () => {
      const result = await scheduleIngredients(
        mockData,
    ...
    it("should throw error when noteId is missing", async () => {
      ...
      await expect(
        scheduleIngredients(dataWithoutNoteId, mockDependencies.logger)
      ).rejects.toThrow(
        "No note ID available for ingredient scheduling"
      );
    });
    ...
    it("should handle errors and log them", async () => {
      ...
      await expect(
        scheduleIngredients(mockData, mockDependencies.logger)
      ).rejects.toThrow("Test error");

      expect(mockDependencies.logger.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "Failed to schedule ingredient processing: Error: Test error"
        )
      );
    });
  });
