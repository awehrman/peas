...
describe("Note Actions Index", () => {
  ...
  describe("registerNoteActions", () => {
    ...
    it("should register actions in the correct order", () => {
      ...
      expect(calls[1]).toEqual(["save_note", expect.any(Function)]);
      expect(calls[2]).toEqual(["schedule_images", expect.any(Function)]);
      expect(calls[3]).toEqual(["schedule_source", expect.any(Function)]);
      expect(calls[4]).toEqual(["schedule_ingredients", expect.any(Function)]);
      expect(calls[5]).toEqual(["schedule_instructions", expect.any(Function)]);
      expect(calls[6]).toEqual(["add_status_actions", expect.any(Function)]);
    });
