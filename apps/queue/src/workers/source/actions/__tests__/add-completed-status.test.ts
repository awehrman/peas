...
// Mock the BroadcastCompletedAction
vi.mock("../../../shared/broadcast-status", () => ({
  BroadcastCompletedAction: vi.fn().mockImplementation(() => ({
...
describe("AddCompletedStatusAction", () => {
  let action: AddCompletedStatusAction;
  ...
  beforeEach(() => {
    action = new AddCompletedStatusAction();
    mockDeps = {
  ...
  describe("execute", () => {
    it("should add completed status when noteId and importId are provided", async () => {
      const input = {
        noteId: "test-note-123",
        importId: "test-import-456",
        sourceId: "test-source-789",
        savedSource: { id: "test-source-789", title: "Test Source" },
      };

      const result = await action.execute(input, mockDeps, mockContext);

      expect(result).toEqual(input);
      expect(BroadcastCompletedAction).toHaveBeenCalledWith();
    ...
    it("should skip completed status when noteId is missing", async () => {
      const input = {
        importId: "test-import-456",
        sourceId: "test-source-789",
        savedSource: { id: "test-source-789", title: "Test Source" },
    ...
    it("should skip completed status when importId is missing", async () => {
      const input = {
        noteId: "test-note-123",
        sourceId: "test-source-789",
        savedSource: { id: "test-source-789", title: "Test Source" },
    ...
    it("should skip completed status when both noteId and importId are missing", async () => {
      const input = {
        sourceId: "test-source-789",
        savedSource: { id: "test-source-789", title: "Test Source" },
      };
  ...
  describe("action properties", () => {
    it("should have correct name", () => {
      expect(action.name).toBe("add_completed_status");
    });
