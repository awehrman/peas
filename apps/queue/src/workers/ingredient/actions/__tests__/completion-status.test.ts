...
describe("CompletionStatusAction", () => {
  ...
  const mockInput = {
    ...
    segmentsSaved: 3,
    parseStatus: "CORRECT",
    normalizedText: "test ingredient",
    stepsCount: 1,
  };
  ...
  describe("execute", () => {
    ...
    it("should log debug information correctly", async () => {
      ...
      expect(mockDeps.logger?.log).toHaveBeenCalledWith(
        `[COMPLETION_STATUS] Received input: noteId=${mockInput.noteId}, importId=${mockInput.importId}, currentIngredientIndex=${mockInput.currentIngredientIndex}, totalIngredients=${mockInput.totalIngredients}`
      );
    });
  });
