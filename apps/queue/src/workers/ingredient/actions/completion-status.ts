...
import { SaveIngredientLineOutput } from "./save-ingredient-line";
...
> {
  ...
  async execute(
    ...
    deps.logger?.log(
      `[COMPLETION_STATUS] Received input: noteId=${noteId}, importId=${importId}, currentIngredientIndex=${input.currentIngredientIndex}, totalIngredients=${input.totalIngredients}`
    );
