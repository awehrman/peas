...
import { CompletedStatusAction } from "../../../core/status-actions/completed-status-action";
import type {
  BaseJobData,
  StatusBroadcaster,
  StatusDeps,
...
describe("CompletedStatusAction", () => {
  let action: CompletedStatusAction;
  ...
  beforeEach(() => {
    action = new CompletedStatusAction();
    mockDeps = {
      statusBroadcaster: {
  ...
  describe("basic functionality", () => {
    ...
    it("should be instance of CompletedStatusAction", () => {
      expect(action).toBeInstanceOf(CompletedStatusAction);
    });
  ...
  describe("execute method", () => {
    it("should broadcast completed status when statusBroadcaster and importId are available", async () => {
      ...
      expect(
        ...
        noteId: "test-note-456",
        status: "COMPLETED",
        message: `Completed ${mockContext.operation}`,
    ...
    it("should broadcast completed status when only importId is available (no noteId)", async () => {
    ...
    it("should not broadcast when statusBroadcaster is not available", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        `[${mockContext.operation}] Completed status for job ${mockContext.jobId}`
      );
    });
    ...
    it("should handle statusBroadcaster error gracefully", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        `[COMPLETED_STATUS] Failed to broadcast: ${error}`
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        `[${mockContext.operation}] Completed status for job ${mockContext.jobId}`
      );
    ...
    it("should always log completion message regardless of broadcast success", async () => {
      const data: BaseJobData & { importId: string } = {
        importId: "test-import-123",
      };
  ...
  describe("integration scenarios", () => {
    ...
    it("should work with custom operation names", async () => {
      ...
      expect(
        ...
        noteId: undefined,
        status: "COMPLETED",
        message: "Completed custom-operation",
    ...
    it("should work with custom job IDs", async () => {
      ...
      expect(
        ...
        noteId: undefined,
        status: "COMPLETED",
        message: `Completed ${customContext.operation}`,
  ...
  describe("error handling", () => {
    it("should handle TypeError from statusBroadcaster", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        `[COMPLETED_STATUS] Failed to broadcast: ${typeError}`
      );
    });
    ...
    it("should handle network errors from statusBroadcaster", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        `[COMPLETED_STATUS] Failed to broadcast: ${networkError}`
      );
    });
